...
protected:
  ...
  // Simple PD gains for effort control
  double kp_{50.0};
  double kd_{5.0};
  ...
  // Maps to make on_activate() generic
  std::unordered_map<
    std::string, std::vector<std::reference_wrapper<hardware_interface::LoanedCommandInterface>> *>
    command_interface_map_ = {
      {hardware_interface::HW_IF_EFFORT, &joint_effort_command_interface_}
    };
  ...
  std::unordered_map<
    ...
    state_interface_map_ = {
      ...
      {hardware_interface::HW_IF_EFFORT,   &joint_effort_state_interface_}
    };
};
